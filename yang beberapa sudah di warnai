...
header <- dashboardHeader(
  ...
  tags$li(
    ...
    style = "position: absolute; left: 40px; margin-top: -23px;",  # Mengangkat ikon sedikit
    tags$a(
    ...
    tags$div(
      ...
      style = "width: 300px; max-height: 300px; overflow-y: auto; white-space: normal;",
...
# Body dengan CSS Custom yang lebih spesifik
body <- dashboardBody(
  tags$head(
    tags$style(HTML("
      body {
      ...
      .welcome-box {
        background: linear-gradient(to right, #f48fb1 0%, #f8bbd0 100%);
        color: #ffffff;
        border-radius: 10px;
        padding: 20px;
      ...
      .navbar, .navbar-light, .main-header .navbar .navbar-nav {
    ...
    tagList(
      lapply(1:nrow(notifications), function(i) {
        tags$div(
          ...
          tags$div(
            tags$span(
              icon("exclamation-circle"), # Ikon untuk notifikasi penting
              style = "color: #ff9800; margin-right: 10px;" # Warna sesuai tingkat kepentingan
            ),
            tags$div(
              tags$p(
                tags$strong(notifications$message[i]), # Pesan notifikasi
                style = "margin: 0;"
              ),
              tags$small(
                class = "float-right text-muted",
                format(notifications$date[i], "%d %b %Y, %H:%M") # Format tanggal lebih ringkas
              )
            )
          )
        )
